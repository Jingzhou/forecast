<template>
  <div class='divination-page'>
    <van-field
      v-model="question"
      label="问题"
      placeholder="请描述你的问题或疑惑"
    />
    <van-button type="primary" block style='margin-top: 4px' @click='divination' :disabled='!question'>求卦</van-button>
    <div class='result-wrap' v-if='!!resultData'>
      <van-button type="primary" @click='unscramble' v-if='!resultData.unscramble' block style='margin-top: 4px'>解卦</van-button>
...
const unscramble = () => {
  ...
  post.gerDivination(data).then(res => {
    if(res.code === '0') {
